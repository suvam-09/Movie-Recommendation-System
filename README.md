...
**Disclaimer**:
- The *similarity_check.pkl* file that houses the distance between movies (vectors) arranged in an array, has a file size greater than 100 MB. I have used the LFS Git Extension for versioning this large file. However, as Heroku doesn't suppost for LFS objects, the app deployment process failed multiple times for me during the initial stage. Reading through several posts on stackoverflow, I decided to use the config vars option available under Heroku server. Heroku lets us externalise configuration i.e., storing data such as encryption keys or external resource addresses in config vars, and later at runtime, these config vars are exposed as environment variables to the application. So I created a key-value pair under my Heroku server, and had it configured as environment variable to my application using a PAT (personal access token) generated under my GitHub account. This helped uploading the LFS objects successfully on the server, post which the deployment completed successfully adding to the expected working of the application.
- With regards to the *requirements.txt* file, initially I had run the *pip freeze* -> *requirements.txt* code under my terminal which listed out all the libraries that would be used in running our application on the server under a file named *requirements.txt*. However, while pushing the changes to the server, there was some error wherein the libraries failed to upload on the server owing to the requirement for a different version of Python for few of these libraries, other than the one being used in this project. So, I modified the *requirements.txt* file by removing all the libraries and mentioning only the libraries that I had imported under my *app.py* file. With this, Heroku automatically decides which version is required for the installation of all the libraries over the server.
